...
struct selection {
	float xmin, ymin;
	float xmax, ymax;
	selection(){ xmin=xmax=ymin=ymax=0; }
	selection(float x, float y, float X, float Y){ xmin=x; xmax=X; ymin=y; ymax=Y; }
	void operator=(selection &O){ xmin=O.xmin; xmax=O.xmax; ymin=O.ymin; ymax=O.ymax; }
};
