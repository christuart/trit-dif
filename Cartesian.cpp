...
static const double default_lin_intervals[NO_LIN_DEFAULTS] =
    {
        1, 2, 5
    };
static const int default_lin_major_steps[NO_LIN_DEFAULTS] =
    {
        5, 5, 2
    };
static const int default_lin_label_steps[NO_LIN_DEFAULTS] =
    {
        10, 5, 4
    };

static const int NO_LOG_DEFAULTS = 3;
static const double default_log_intervals[NO_LOG_DEFAULTS] =
    {
        1, 2, 5
    };
static const int default_log_major_steps[NO_LOG_DEFAULTS] =
    {
        5, 5, 2
    };
static const int default_log_label_steps[NO_LOG_DEFAULTS] =
    {
        10, 5, 2
    };
...
void Ca_X_Axis::draw()
{
    ...
    if (damage()&(FL_DAMAGE_ALL|CA_DAMAGE_ALL))
    {
        ...
        while(next_tick(tick_index, tick_value, tick_order, _interval))
        {
            ...
            if(scale_&CA_REV)
            {
                if(_pos<max_pos_-canvas_->border())
                    break;
                if(_pos>min_pos_+canvas_->border())
                    continue;
            }
            else
            {
...
void Ca_Y_Axis::draw()
{
    ...
    if (damage()&(FL_DAMAGE_ALL|CA_DAMAGE_ALL))
    {
        ...
        int l=0;
        int _x=0;
        int _w,_h; //temporary coordinates for ticks
        double _pos,_y;
        fl_clip(x()+Fl::box_dx(box()),y()+Fl::box_dy(box()),w()-Fl::box_dw(box()),h()-Fl::box_dh(box()));
        fl_color(axis_color_);
...
int Ca_Canvas::handle(int event)
{
    ...
    switch(event) {
        ...
        case FL_DRAG:
			...
			y1 = Fl::event_y();
			fl_color(FL_RED);
			fl_overlay_rect(x0, y0, x1-x0, y1-y0);
			return 1;
        case FL_RELEASE:
			x1 = Fl::event_x();
			y1 = Fl::event_y();
			fl_overlay_clear();
			sel_rect.xmin = current_x()->value(x0);
			sel_rect.xmax = current_x()->value(x1);
			sel_rect.ymin = current_y()->value(y0);
			sel_rect.ymax = current_y()->value(y1);
			do_callback();
			return 1;
        default:
